...
%I also thank my parents, who still have no idea what this project is about, for their suggestions and encouragement.
...
\item{\textit{Xilinx PicoBlaze \cite{picoblaze}.} }
\end{itemize}\improvement{Reference and needs more}
...
The design, implementation, and verification of the processor design consist of the following project stages:

\begin{description}[style=nextline]
\item [Stage 1.2: Processor specification, architecture, ISA]
As discussed in section \ref{current_impl}, existing embedded processors have been researched and compared in order to determine a suitable and realistic specification for the processor  design.
...
\section{Registers}
\scname{} has a total of \todo{8 and Rx}6 addressable, read and write, registers. These registers are identified by letters A through H.

\subsection{General Purpose Registers}
Registers \todo{Compiler code calls registers Ax-Hx}A through E are designed for general purpose use and are safe to store user values over the run-time of the processor.
...
\todo[inline]{compiler and processor objectives already reviewed in their own chapters}
...
\todo[inline]{Need more project management drawings (requirements, time plans, stages, plans, etc.}
