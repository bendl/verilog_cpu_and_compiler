...
\item[GitHub (with git repository metadata)]{\info{How early can I make this repository public?}\url{https://github.com/bendl/prco304}}
...
In addition to FPGA-based implementation, the Cortex-A9 is also designed for silicon based SoC designs. The processor is implemented in many commercial SoC products such as \todo{cite}Broadcom's BCM11311, \todo{cite}STMicroelectronics' SPEAr1310, and Apple's A5 mobile chip. }
...
\unsure{These are slightly different to the PID, is this OK?}
...
The \scname{} processor is \textbf{not} designed to run in high-reliability or safety-critical environments that require established safety standards, such as the UK Defence Standard 00-56 \cite{defstan_0056_2} and IEC 61508 \cite{iec61508}.\improvement{Cite actual standards documents, not papers describing them.}
...
This project will be split into 4 main stages: First is the requirements and information gathering as well as finalising core and compiler features (such as registers and instructions). The second stage consists of the design, implementation, and verification  of the embedded processor core.

Split project into 2 independent parts - core and compiler

A combination of prince2 and agile methodologies.

Employ agile sprints

\improvement[inline]{Show github kanban board}

\section{Time Management}
\label{sect:proj_time}
\improvement[inline]{Show gantt chart}
...
\def\arraystretch{1.5}%  1 is the default, change whatever you need
	...
	\\ \hline
	1 & SR\_S & Set if the result of a CMP instruction is signed (greatest bit is 1).
	\\ \hline
	2 & SR\_O & Set if the result's 17'bit of a CMP instruction's is set.
	\\ \hline
    \end{tabularx}
...
\\\\
The \scname{} processor implements it's own fixed 16-bit little-endian instruction set.
\todo{More}

\subsection{Instruction Types}
It was decided to support the following 3 types of instructions. This allows for a wide range of instruction operands, such as more registers or larger immediate values.
...
\textit{Instructions Per Cycle} (IPC) and \textit{Million Instructions Per Second} (MIPS) are two measurements used to measure the performance of a processor.
...
The \scname{} processor is similar to AMD's Am386 (1991) processor with 9 MIPS and 0.225 IPC when run at 40 MHz \todo{Cite actual processor datasheet, not a magazine...}\cite{am386}.
...
The \scname{} compiler employs continuous integration testing to show if a code commit breaks certain functionality. The continuous integration system service used is Travis CI \todo{fix}\cite{travisci}. Every time a commit is made to the repository, Travis CI pulls the commit and runs the \verb|test/travis-ci.sh| script. This script runs the compiler and emulator on each \verb|test/*.prco| file and checks the return code. If the return code does not match the expected functionality, the test fails and the developer can investigate further by enabling more debug prints with the \verb|-D| parameter. These tests will generate a report detailing passed and failed tests with their actual and expected outputs. 
...
A detailed specification of the new \scname{} processor, describing the feed-forward pipeline, timings 

A new processor architecture, with a feed-forward pipeline, 

In addition, methods to test and verify both processor and compiler have been 
...
Once the processor was able to support a program counter, instructions could be fetched, executed, data written, and control execution of the processor. With the addition of comparison and branching instructions, CMP and JMP, dynamic execution and program flow was introduction. This allows the processor, without knowledge of high-level concepts such as \textit{if} and \textit{for} loop statements, to perform complex logic sequences by executing primitive instructions.
...
As discussed in section {\nameref{ise_impl}}, the \scname{} processor has been successfully implemented on the MiniSpartan6+ development board. Although successful, the implementation did take up more FPGA slice resources, likely caused by poorly designed register and RAM components that did not infer to physical FPGA block RAMs. This would result in smaller user FPGA logic space.


\item[C4. To provide a high-level context-free code compiler to run user-code on the processor.]
A C-life grammar compiler was designed, implemented, and verified. The compiler accepted a context-free grammar similar to C's grammar. The grammar featured terminal symbols for variables and arithmetic operators, and non-terminal  symbols for complex patterns like \textit{for} loops and function definitions.
...
The combination of PRINCE2 with Agile was a success. (using the PID, plans, and risks) (weekly sprints, constant reviews)
