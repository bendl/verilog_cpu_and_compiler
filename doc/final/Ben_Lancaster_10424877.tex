...
\section{Testing and Verification}\label{sect:core_test}
...
The compiler's functional requirements are detailed in {\nameref{sect:compiler_func_req}} \ref{sect:compiler_func_req}. 
...
\section{AST Generation}\label{sect:compiler_ast}
...
\subsection{Variables}\label{sect:compiler_cg_vars}
...
\end{minted}
        \captionof{figure}{Input high-level code showing 3 variable declarations and references.}\label{fig:compiler_cg_vars_high}
    \end{minipage}
...
\subsection{PUSH and POP}\label{sect:compiler_cg_push_pop}
...
\newpage
\subsection{Appendix B. Compiler Functional Requirements}\label{sect:compiler_func_req}
This section details the functional requirements (F) and their technical dependencies of the compiler to allow users to produce complete and functional programs. Figure \ref{fig:compiler_dependencies} below breaks down each functional requirement to show technical dependencies. For example, to support 'F4 User-defined program flow', the compiler needs to support control sequences such as 'for' and 'if' statements which themselves require implementation of 'variables' and 'conditional branching', and so on.

\begin{enumerate}[label=F\arabic*.]
\item{\textbf{Program flow manipulation.} The compiler will support the creation of divergent segments in the program code through the use of unconditional and conditional jumps.}
...
\begin{figure}[H]
\begin{center}
\includegraphics[scale=0.4]{compiler_dependencies}
\end{center}
...
\subsection{Appendix B. CLI Arguments}\label{sect:compiler_cli}

\subsection{Appendix B. Compiler Sequence Diagram}\label{sect:compiler_seq_uml}
\begin{figure}[H]
\begin{center}
