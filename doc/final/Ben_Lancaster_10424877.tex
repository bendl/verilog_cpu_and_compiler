...
\hline 
\end{tabular}
\caption{The 2 instruction format types used by the \scname{} processor.}
\end{table}
...
An initial requirement for the \scname{} compiler was to support three types of variables: global, local, and parameter variables. Due to time constraints and unforeseen \textit{"sleeper"} bugs only local variables have been implemented as of compiler version 2.40 (05/04/2018). Global variable allocation has been implemented in the assembler but declaring or referencing global variables has not been implemented.
...
Strings are an extended deliverable for the \scname{} compiler but was added to better demonstrate the capabilities of the compiler and processor. Strings in modern OS executable file formats, such as ELF and Windows' PE formats, store explicitly declared strings as null-terminated ASCII strings in the String Table or \verb|.text| section of the executable file.
...
\hline 
IF statements & Yes & 1.30 & \\ \hline 
IF ELSE statements & Yes & 1.30 & \\ \hline 
...
\begin{tabularx}{\textwidth}{|p{4cm}|c|c|X|}
\hline 
Assembly text parsing & No & X & Only the CFG text parser for the grammar described in section \ref{sect:compiler_grammar} is present. This would be a desirable feature.
\\ \hline 
...
\\ \hline 
Assembler memory layout & Limited & 2.10 & First instructions (low memory) jump into \textit{middle} memory where \verb|main| function is located. Low memory consists of \textit{global values} (like strings and global variables). High memory is reserved for stack management. 
\\ \hline 
\end{tabularx} 
\end{table}
...
\subsubsection*{Command Line Interface Arguments}
