...
\section{Background}
\subsection{Existing Embedded Processor}\label{current_impl}
...
This has been achieved. A number of applications have been written for the processor (located in \verb|prco_compiler/test/tests/|) ranging from string manipulation, integer division, and console printing. The combination of these programs can result in a wide range of large and complex applications.

\item [CPD2. Provide a 16-bit instruction set capable of supporting simple programs.]
This has been achieved. The complete instruction listing can be found in {\nameref{appendix:isa}}.

\item [CPD3. Support simple arithmetic and bitwise operations.]
This has been achieved. Although only addition and subtraction have been implemented in hardware, multiplication, division, and modulo, can be performed in software using multiple simple instructions.

\item [CPD4. Operate on 16-bit data sizes.]
This has been achieved. All register sizes, memory cell sizes, and ALU outputs are of 16-bit words.
Operands using only 8-bits are bit-extended to 16-bits by the ALU.

\item [CPD5. Fully synthesize-able on FPGA hardware.]
This has been achieved. Section \ref{ise_impl} details the implementation on the XC6SLX9 FPGA.

\item [CPD6. Implement a simple pipeline architecture.]
This has been achieved. The pipeline is described and reviewed in section \ref{core:pipeline}  {\nameref{core:pipeline}}.
\end{description}
